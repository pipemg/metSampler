function compileOptGpSampler()
    ...
    cppFiles = [    ' ../src/CbModel.cpp', ...
                    ...
                    ' ../src/mxValidate.cpp', ...
                    ' ../src/optGpSampler.cpp', ...
                    ' ../src/optGpSamplerMex.cpp'];                    

    cppIncludeDirs =   ' -I"../include"';
    outputFile = ' cOptGpSampler';
    ...
    cmd = ['mex -v -largeArrayDims COMPFLAGS="\$COMPFLAGS" CXXFLAGS="\$CXXFLAGS -fopenmp" LDFLAGS="\$LDFLAGS -fopenmp" ', ...
            ...
            ' -L"', matlabLibs, '" -lmwblas -lmwlapack -lut -ldl'];
        
    eval(cmd);
    fprintf('optGpSampler.%s successfully compiled! \n', mexext);
    system(sprintf('mv cOptGpSampler.%s ../../', mexext));
